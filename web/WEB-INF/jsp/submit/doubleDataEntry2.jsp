...
<form id="mainForm" name="crfForm" method="post" action="DoubleDataEntry2">
...
function DisplaySectionTabs()
{
...

    {
        ...
        url = "DoubleDataEntry2?eventCRFId=" + <c:out value="${section.eventCRF.id}"/> + "&sectionId=" + sectionId + "&tab=" + TabID + "&exitTo=${exitTo}";
...
<td>&nbsp;
    <div class="formfieldM_BG_noMargin"><select class="formfieldM" name="sectionName" size="1" onchange="gotoLink();">
        ...
        <c:forEach var="sec" items="${toc.sections}" >
            <c:set var="tabUrl" value = "DoubleDataEntry2?eventCRFId=${section.eventCRF.id}&sectionId=${sec.id}&tab=${tabCount}&exitTo=${exitTo}"/>
            ...
            <c:forEach var="respOption" items="${bodyItem.metadata.responseSet.options}">
                <td class="aka_padding_norm aka_cellBorders_dark <c:out value="${extraClass}"/>">
                    ...
                    <c:import url="../submit/showGroupItemInput.jsp">
                        ...
                        <c:param name="originJSP" value="doubleDataEntry2"/>
                    </c:import>
                </td>
            </c:forEach>
        </c:when>
