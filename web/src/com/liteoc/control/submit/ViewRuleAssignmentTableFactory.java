...
import com.liteoc.control.AbstractTableFactory;
import com.liteoc.control.DefaultActionsEditor;
import com.liteoc.control.OCTableFacadeImpl;
...
import com.liteoc.domain.rule.action.RuleActionRunBean;
...
public class ViewRuleAssignmentTableFactory extends AbstractTableFactory {
    ...
    private class ActionSummaryCellEditor implements CellEditor {
        ...
        private void appendDestProps(HtmlBuilder builder, 
                List<com.liteoc.domain.rule.action.PropertyBean> propertyBeans) {
            if(propertyBeans!=null && propertyBeans.size()>0) {
                String s = "";
                for(com.liteoc.domain.rule.action.PropertyBean p : propertyBeans) {
                    s += p.getOid().trim() + ", ";
                }
                s = s.trim(); s = s.substring(0,s.length()-1);
